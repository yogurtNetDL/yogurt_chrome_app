...
class SocketFetch extends ArcEventSource {
  ...
  constructor(url, opts) {
    console.log("Create SocketFetch, url",url)
    super();
    this._logs = [];
    this._current_download = 0;
  ...
  _processBody(data) {
    ...
    if (this._connection.chunked) {
      ...
      if (!this._connection.body) {
        this._content_length = this._connection.contentLength;
        this._current_download = 0;
        this.log('Creating new body');
        this._connection.body = new Uint8Array(Math.max(this._content_length,data.length));
        this._connection.body.set(data,this._current_download);
        this._current_download = this._current_download + data.length;
        if (this._current_download >= this._content_length) {
          this.log('Response ready. Calling it.');
          this.onResponseReady();
        }
        //console.log("new body",this._connection.body,this._current_download,this._content_length)
      } else {
        //this._content_length = this._connection.contentLength;
        this._connection.body.set(data,this._current_download);
        this._current_download = this._current_download + data.length;
        //console.log("follow up",this._current_download,data.length)
  ...
  decompressData(data) {
    ...
    if (ce.indexOf('gzip') !== -1 || ce.indexOf('deflate') !== -1) {
      return new Promise((resolve, reject) => {
        let workerUrl = 'bower_components/socket-fetch/decompress-worker.js';
        if (SocketFetchOptions.importUrl) {
          ...
          // demo, test
          workerUrl = 'bower_components/socket-fetch/decompress-worker.js';
        }
